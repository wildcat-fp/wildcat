 ...
 *
 * @param <For> The type constructor (a type that takes another type as a parameter). This should be a type implementing the `Kind.k` interface.
...
public interface Kind<For extends Kind.k, T extends @NonNull Object> {
     ...
     *
     * @param <A> The target type to cast to, which must extend `Kind<For, T>`.
