...
public interface Functor<For extends Functor.k, T> extends Invariant<For, T> {
    ...
    <B> @NonNull Kind<For, B> map(@NonNull Functor<For, T> a, @NonNull NonNullFunction<? super T, B> f);

    @Override
    default <SecondValue, Out extends @NonNull Invariant<For, T>> Out imap(final @NonNull Kind<For, T> a,
