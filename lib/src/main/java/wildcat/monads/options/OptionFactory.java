...
public sealed interface OptionFactory
  permits ImmediateOption.Factory, LazyOption.Factory {
    default <T extends @NonNull Object> Option<T> when(final boolean condition, final T value) {
      if (condition) {
    ...
    default <T extends @NonNull Object> Option<T> when(final boolean condition, final Supplier<? extends T> supplier) {
      if (condition) {
  ...
  default <T, U extends @NonNull Object> Option<U> lift(final Function<? super @NonNull T, ? extends U> function, final @Nullable T value) {
    if (value == null) {
      return empty();
    } else {
  ...
  <T extends @NonNull Object> Option<T> present(Supplier<? extends T> supplier);
}